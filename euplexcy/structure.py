...
def text_parts(doc):
    ...
    while True:
        ...
        if enacting_start_match is not None:
            ...
            raise exceptions.StructureException("Text does not appear to be in a format that can be handled by this function.")
    ...
    while True:
        ...
        middle_end_match = re.search (f'(?<=.{{{pos_last_article}}}).*?(^)(?=(?:[\(\[][0-9]+[\)\]])|(?:[A-Z]{{3,}}))', text, flags=re.DOTALL|re.MULTILINE)
    ...
    if middle_end_match is not None:
        middle = doc[_to_token(middle.start_char):_to_token(middle.start_char+middle_end_match.end())]
        back = doc[_to_token(middle.start_char+middle_end_match.end()):_to_token(text_end_char)]
    else:
        back = None
    ...
    if (recitals is None and not no_recitals) and (citations is None and not no_citations):
