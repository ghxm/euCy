...
class Elements:
    ...
    def __init__(self):
        ...
        if not Span.has_extension("element_type"):
            ...
            Span.set_extension ("element_pos", default=None)  # the index of an element
...
def citations(doc_citations):
    ...
    char_token = utils.chars_to_tokens_dict(citations, input  = "as_ref", output = "as_ref")
    ...
    if isinstance(doc_citations, Doc):
        ...
        citation_list = [doc_citations[utils.char_to_token(mat.start(), char_token):utils.char_to_token(mat.end(), char_token, alignment_mode="expand")] for mat in citation_matches]
...
def articles(doc_articles):
    ...
    char_token = utils.chars_to_tokens_dict (articles, input="as_ref", output="as_ref")
    ...
    article_id_matches = [ma for ma in article_id_matches if len (ma.group (0)) > 4]
    ...
    for i, m in enumerate(article_id_matches, start=0):
        ...
        if isinstance (doc_articles, Doc):
            article_span = doc_articles.char_span.char_span (article_start, article_end, alignment_mode="expand")
        else:
            article_span = doc_articles[utils.char_to_token (article_start, char_token):utils.char_to_token (article_end, char_token, alignment_mode="expand")]
...
def article_elements(doc_article):
    ...
    if not isinstance(doc_article, (Doc, Span)):
        raise TypeError (str (type (doc_article)) + " type not supported. Please pass a Doc or Span object.")

    def _paragraphs(article):

        par_char_token = utils.chars_to_tokens_dict (article, input="as_ref", output="as_ref")
    ...
    def _subparagraphs(par):
        ...
        subpar_char_token = utils.chars_to_tokens_dict (par, input="as_ref", output="as_ref")
    ...
    def _points(subpar):
        ...
        point_char_token = utils.chars_to_tokens_dict (subpar, input="as_ref", output="as_ref")
    ...
    def _indents(subpar):
        ...
        indent_char_token = utils.chars_to_tokens_dict (subpar, input="as_ref", output="as_ref")
